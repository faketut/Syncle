...
在项目根目录的 `local.properties` 文件中补充您的业务服务端 API 地址及 LiveKit 配置：
```properties
# Backend & LiveKit Configuration
syncle.backend_api_url=https://api.syncle.work/v1
...
- `MapRepositoryTest.kt`: 验证对服务端 JSON 地图元数据的正确解析与内存缓存能力。
